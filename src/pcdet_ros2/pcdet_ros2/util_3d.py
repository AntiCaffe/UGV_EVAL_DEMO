...
class ExtendedKalmanFilterXYZWH:
    def __init__(self):
        """
        기존:
            self.ndim = 6  # x, y, z, w, h, l
            self.dim_x = 3 * self.ndim
            self.dim_z = self.ndim

        수정 후 (yaw 추가 및 등속도 모델):
            self.ndim = 7
            -> 상태 벡터: [x, y, z, w, h, l, yaw,
                           vx, vy, vz, vw, vh, vl, vyaw]
        """
        self.ndim = 7  # (x, y, z, w, h, l, yaw)
        self.dt = 0.1  # 시간 간격 (10Hz)
        self.dim_x = 2 * self.ndim  # 14 (pos와 vel)
    ...
    def f_jacobian(self, state):
        """
        f(state)의 자코비안 행렬 (14x14)를 계산.
        등속도 모델의 경우:
            F = [I, dt * I; 0, I]
        """
        dt = self.dt
        F = numpy.eye(self.dim_x)
        F[:self.ndim, self.ndim:2*self.ndim] = dt * numpy.eye(self.ndim)
        return F

    def h(self, state):
        """
        관측 함수 (선형): 상태의 앞 7개 요소만 관측 가능.
        """
        return numpy.dot(self._update_mat, state)

    def initiate(self, measurement):
        """
        측정값으로부터 초기 상태(mean)와 공분산(covariance)를 설정.
        ...
        ref_scale = measurement[3]  # width를 기준 스케일로 사용
