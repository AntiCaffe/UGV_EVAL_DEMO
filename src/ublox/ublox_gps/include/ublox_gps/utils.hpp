...
static inline void declareRosParameterByType(
  rclcpp::Node* node, const std::string& name, rclcpp::ParameterType type)
{
  rcl_interfaces::msg::ParameterDescriptor param_desc;
  param_desc.name = name;
  param_desc.type = static_cast<uint8_t>(type);
  param_desc.description = name;
  node->declare_parameter(name, rclcpp::ParameterValue(), param_desc);
}
